...
var tmplInternal = template.Must(template.New("render").Parse(`// generated by {{.CmdLine}}; DO NOT EDIT
...
var tmplEditorImpl = template.Must(template.New("render").Parse(`// generated by {{.CmdLine}}; DO NOT EDIT
...
func registerPluginEvents(client *rpc.Client, e wicore.EventRegistry) wicore.EventListener {
	return wicore.MultiCloser{ {{range .Events}}
		e.Register{{.Name}}(func({{.Params.Flat $.CurrentPkg}}) {
			packet := wicore.Packet{{.Name}} { {{.Params.Names}} }
			out := 0
...
var tmplPluginImpl = template.Must(template.New("render").Parse(`// generated by {{.CmdLine}}; DO NOT EDIT
...
func (er *eventTriggerRPC) Trigger{{.Name}}RPC(packet wicore.Packet{{.Name}}, ignored *int) error {
...
func load(inputFile string, inputType string) (*tmplData, error) {
	...
	return &tmplData{
		...
		CmdLine:    "go run ../tools/wi-event-generator/main.go " + strings.Join(os.Args[1:], " "), // Skip os.Args[0] since it may point into $TMP.
		CurrentPkg: "wicore",
		Events:     events,
		Trigger:    "Trigger",
	}, err
...
func mainImpl() error {
	...
	t.CurrentPkg = "wicore"
	t.Trigger = "Trigger"
	if err := generate(t, tmplInternal, "event_registry_internal.go"); err != nil {
		return err
	}
	return nil
}
