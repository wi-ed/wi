...
type editor struct {
	...
	keyBuffer     string                        // TODO(maruel): Move elsewhere?
...
func (e *editor) onTerminalKeyPressed(key key.Press) bool {
	...
	if key.IsValid() {
		...
		if keyName == "Enter" {
			...
			e.ExecuteCommand(e.ActiveWindow(), e.keyBuffer)
			e.keyBuffer = ""
		} else if key.IsMeta() {
			// Convert the key press into a command. The trick is that we don't know
			// the active window, there could be commands already enqueued that will
			// change the active window, so using the active window directly or
			// indirectly here is an incorrect assumption.
			cmdName := wicore.GetKeyBindingCommand(e, e.KeyboardMode(), key)
			if cmdName != "" {
